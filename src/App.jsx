...
const SECTIONS = [
  { id: 'actif', label: 'Actif', fields: [
    ...
    { key: 'classification_batiment', label: 'Classification réglementaire (Code du travail / ERP / IGH)' },
  ]},
  ...
  { id: 'restitution', label: 'Conditions de restitution', fields: [
    { key: 'restitution_etat', label: 'État de restitution requis' },
    { key: 'restitution_delai_edl', label: "Délai pour l'état des lieux de sortie" },
    { key: 'restitution_conditions_specifiques', label: 'Conditions spécifiques' },
  ]},
]
...
- duree_totale: duree totale du bail (date_effet a date_fin). duree_ferme: duree pendant laquelle le preneur ne peut pas resilier ; si mentionne explicitement (chiffre en toutes lettres), utiliser cette valeur ; si break_options, c'est l'intervalle date_effet->premiere break. IMPORTANT: si duree_ferme < duree_totale et break_options est vide, ajouter dans break_options la date correspondant a date_effet + duree_ferme (premiere sortie possible).
  CAS LE PLUS SIMPLE ET LE PLUS FREQUENT, A NE PAS MANQUER: quand la phrase qui donne duree_totale qualifie ELLE-MEME cette duree du mot "ferme(s)" directement accole (ex: "consenti... pour une duree de 6 (six) annees entieres et consecutives FERMES a compter du..."), duree_ferme = duree_totale IMMEDIATEMENT, sans avoir besoin d'aucune clause de renonciation separee — "fermes" qualifie ici directement le chiffre de duree_totale lui-meme, ce qui EST la valeur chiffree explicite visee par la premiere phrase de cette regle. Ne pas laisser ce cas retomber sur le "cas general" (regle 1 ci-dessous) faute de reconnaitre "fermes" accole a la duree comme une valeur explicite a part entiere.
...
- break_options: liste COMPLETE et EXHAUSTIVE de toutes les dates auxquelles le PRENEUR peut effectivement sortir avant le terme. REGLE PRIORITAIRE (avant toute autre regle de calcul ci-dessous) : une echeance triennale a laquelle le preneur a EXPLICITEMENT renonce par son nom/rang ("renonce a la faculte de donner conge pour l'expiration de la premiere/deuxieme/troisieme... periode triennale", "renonce a la 3eme et 6eme annee", etc.) ne doit JAMAIS figurer dans break_options, MEME SI elle correspond a un multiple standard de 3 ans que la regle generale ci-dessous (multiples de 3 depuis date_effet) generarait autrement automatiquement — une echeance renoncee est purement et simplement supprimee de la liste, pas juste deplacee. Exemple: renonciation aux 1ere ET 2eme periodes triennales sur un bail de 9 ans → 3 ans et 6 ans sont tous deux exclus de break_options ; seule reste l'echeance du terme (qui, coincidant avec date_fin, n'est pas une vraie sortie anticipee : dans ce cas break_options reste VIDE, PAS ["6 ans", "9 ans"] ni meme ["9 ans"]).
  ATTENTION FACULTE "A TOUT MOMENT" (SANS DATE FIXE) — NE JAMAIS INVENTER DE DATE: quand le texte accorde une faculte de resiliation EXCEPTIONNELLE/personnelle "a tout moment" (formulations frequentes: "a tout moment", "a n'importe quel moment", "sans attendre l'expiration d'une periode triennale"), generalement SOUS CONDITION (motif a justifier: suppression/concentration/fusion de service, cessation d'activite, etc.), cette faculte n'a PAR DEFINITION AUCUNE date fixe calculable — NE JAMAIS calculer ou deviner une echeance triennale (annee 3, 6, 9...) pour la representer dans break_options, meme si un preavis en mois est precise juste apres. NE RIEN AJOUTER a break_options pour cette clause : la decrire uniquement dans conditions_break (texte libre complet, y compris le motif exige et le preavis). Exemple a NE PAS produire : bail de 6 ans dont le seul mecanisme de sortie anticipee est "le Preneur pourra resilier le Bail a tout moment moyennant un preavis de 12 mois... sous reserve de justifier un motif de suppression, concentration ou fusion du service" → break_options DOIT rester vide (et duree_ferme = 6 ans si la duree totale est elle-meme qualifiee de "ferme", voir regle duree_ferme) — surtout PAS une date calculee comme si "12 mois de preavis" impliquait une premiere sortie a l'annee 3.
...
- depot_garantie_montant / depot_garantie_duree_mois / depot_garantie: depot_garantie_montant = montant chiffre du depot de garantie classique SI le bail l'exprime en euros (montant fixe ou resultant d'un calcul deja fait dans le texte). depot_garantie_duree_mois: SEPAREMENT, si le bail exprime (aussi ou uniquement) ce depot en nombre de mois de loyer (formulation frequente: "depot de garantie egal a X mois de loyer HT/HC", "correspondant a X mois de loyer principal"), isoler ce nombre ici au format court, ex: "3 mois de loyer HT/HC". Les deux champs montant et duree_mois peuvent coexister (certains baux donnent les deux) ou n'exister que l'un des deux. ATTENTION CAS FREQUENT: quand la clause dit que le depot de garantie est SUBSTITUE/REMPLACE par une GAPD ("le Preneur substitue au depot de garantie une garantie autonome a premiere demande correspondant a X mois de loyer"), le nombre de mois qui suit appartient a la GAPD (gapd_duree_mois), PAS au depot de garantie classique — laisser depot_garantie_montant et depot_garantie_duree_mois a null dans ce cas, meme si "X mois" apparait dans la meme phrase que "depot de garantie". depot_garantie: texte des modalites completes (conditions de restitution, indexation du depot, mention de la substitution le cas echeant, etc.).
- gapd_montant / gapd: si le bail prevoit, EN COMPLEMENT du depot de garantie classique (depot_garantie_montant/depot_garantie), une GARANTIE AUTONOME A PREMIERE DEMANDE (GAPD) — une garantie bancaire distincte ou complementaire, souvent mobilisable "a premiere demande" independamment de toute contestation, generalement mise en place par un etablissement bancaire au profit du Bailleur. NE PAS confondre avec le simple depot de garantie (qui reste dans depot_garantie_montant/depot_garantie) ni avec un cautionnement personnel/solidaire (qui va dans le champ garant). gapd_montant: montant chiffre brut de la GAPD si mentionne. gapd: texte des modalites completes (etablissement emetteur si nomme, duree de validite, conditions de mobilisation, articulation avec le depot de garantie classique). null pour les deux champs si aucune GAPD n'est mentionnee — la tres grande majorite des baux n'en ont pas, ne pas en deduire une simplement parce qu'un depot de garantie classique existe. RAPPEL: des que le champ gapd est renseigne (non-null), penser a inclure sa page dans _pages (cle "gapd") comme pour tout autre champ simple renseigne — cette clause est facile a oublier car souvent absente, ne pas la sauter une fois qu'elle est bien presente.
- gapd_duree_mois: UNIQUEMENT si une GAPD existe (voir gapd_montant/gapd ci-dessus). Isole le nombre de mois de loyer que represente le montant de la GAPD, quand le bail l'exprime ainsi (formulation frequente: "garantie autonome a premiere demande d'un montant egal a X mois de loyer HT/HC", "correspondant a X mois de loyer principal", ou "le Preneur substitue au depot de garantie une GAPD correspondant a X mois de loyer" — dans ce dernier cas de substitution, X mois va ICI, pas dans depot_garantie_duree_mois). Format court, ex: "3 mois de loyer HT/HC" ou juste "3 mois" si l'assiette n'est pas precisee. null si la GAPD est exprimee uniquement en montant fixe sans reference a un nombre de mois de loyer, ou si aucune GAPD n'existe.
...
- restitution_etat: etat PHYSIQUE dans lequel les locaux doivent etre rendus a la sortie (independant de remise_en_etat, qui ne concerne que l'indemnite forfaitaire payee a la signature). Resumer brievement la clause decrivant l'etat de restitution attendu, ex: "en bon etat d'entretien et de reparations locatives", "en parfait etat, tel que livre a l'origine", "sans obligation de remise en etat, degradations locatives normales exceptees". null si non precise dans le bail.
- restitution_delai_edl: delai mentionne pour realiser l'ETAT DES LIEUX DE SORTIE (contradictoire ou non), exprime tel que redige dans le bail — ex: "1 mois avant la date d'expiration du bail", "a la date de liberation effective des locaux". Distinct du preavis de conge (notice) : ce delai concerne specifiquement l'organisation de l'etat des lieux de sortie, pas la notification du conge. null si non precise.
- restitution_conditions_specifiques: toute condition PARTICULIERE relative a la restitution des locaux, notamment (liste non exhaustive) : exoneration totale ou partielle de l'obligation de remise en etat MOYENNANT le versement d'une indemnite liberatoire (dans ce cas, resumer brievement ici ET renseigner le detail chiffre dans remise_en_etat si c'est la meme clause — ne pas dupliquer les chiffres, juste renvoyer d'un mot, ex: "Exoneration moyennant indemnite liberatoire (voir Remise en etat)"), exoneration SANS contrepartie financiere, obligations specifiques de depose de cloisons/enseignes/installations du preneur, clause de non-recours du bailleur pour vetuste normale, etc. null si aucune condition particuliere au-dela du droit commun.
...
- date_effet_avenant: UNIQUEMENT si l'avenant fixe explicitement (ou permet de deduire sans ambiguite) une date a laquelle SES modifications prennent effet — qui peut differer de la date de signature (effet retroactif ou differe explicitement stipule). NE JAMAIS y recopier la date de signature par defaut quand aucune prise d'effet propre n'est stipulee — laisser null dans ce cas (le tri par date_signature_avenant en absence de date_effet_avenant est deja gere par ailleurs).
...
Regles strictes:
- pertinent: true UNIQUEMENT si le document est un bail commercial original ou un avenant a un bail commercial (y compris s'il est intitule "protocole" ou "accord" mais qu'il modifie REELLEMENT les conditions d'un bail: loyer, franchise, duree, surfaces). false dans TOUS les autres cas: side letter TVA, courrier simple, facture, plan, etat des lieux, diagnostic energetique, acte de cautionnement autonome, police d'assurance, mandat de gestion, proces-verbal d'assemblee, ou tout document qui ne modifie pas lui-meme les conditions d'un bail commercial. ATTENTION CAS FREQUENT ET TROMPEUR PAR SON TITRE: un document intitule "PROTOCOLE D'ACCORD" (ou "accord", "lettre") qui traite UNIQUEMENT du traitement fiscal/TVA d'une franchise de loyer DEJA consentie dans le bail (ex: engagements reciproques en cas de redressement TVA de l'administration fiscale sur la franchise, obligations de facturation croisee de TVA) est UNE SIDE LETTER TVA — pertinent:false — MEME s'il mentionne abondamment le mot "franchise" et son montant/duree en detail, et meme s'il ne s'intitule pas explicitement "side letter". Le signal decisif: chercher une clause du type "la presente n'apporte aucune modification au Bail" ou equivalent — si elle existe, c'est une side letter TVA, pas un avenant, quel que soit le titre du document.
...
- raison: explication courte si pertinent:false (ex: "side letter TVA sur franchise deja consentie, n'apporte aucune modification au bail", "etat des lieux", "diagnostic energetique")
...
function PageJumpIcon({ item, pages, field, page: pageProp, title }) {
  const page = pageProp ?? pages?.[field]
...
async function detectAnnexPageViaClaude(base64) {
  const prompt = `Ce PDF est un bail commercial ou un avenant, scanné (photocopié). Il se termine typiquement, juste après la page de signature, par une page listant les documents annexés, au format "Annexe 1 : ...", "Annexe 2 : ...", etc. (parfois "Annexe n°1", numérotation possiblement différente).

Attention à ne pas confondre cette page avec une simple RÉFÉRENCE à une annexe glissée en prose ailleurs dans le contrat (ex. "...dont les coordonnées sont jointes en Annexe 1 de l'Avenant" au milieu d'un article) : ce n'est PAS la page recherchée. La vraie page listant les annexes présente plusieurs entrées "Annexe N : Titre" à la suite les unes des autres, sans autre texte contractuel autour, et se trouve après la page de signature. Après cette page viennent les annexes elles-mêmes (K-bis, plans, diagnostics, RIB, devis, relevés de charges, etc.).
...
// Format for display
function fmtEur(val) {
  // Un montant marqué "recalcul = ..." (déduit, pas écrit tel quel dans le
  // bail) doit rester visible tel quel — le reformater via parseAmount
  // effacerait ce préfixe et perdrait l'info que c'est une valeur déduite.
  if (typeof val === 'string' && /recalcul\s*=/i.test(val)) return val
...
function buildExcelColumnGroups() {
  ...
  return [
    ...
    { label: 'Charges', cols: ['Charges TEOM'] },
    { label: 'Garanties', cols: ['Depot garantie montant', 'Depot garantie duree mois', 'Depot garantie modalites', 'GAPD montant', 'GAPD duree mois', 'GAPD modalites'] },
...
function buildExcelRow(item, bailParentName, bailParentData) {
  ...
  return [
    ...
    // Si date_effet est vide mais définie par un événement futur (date_effet_condition),
    ...
    amt(d.depot_garantie_montant), v(d.depot_garantie_duree_mois), v(d.depot_garantie),
    amt(d.gapd_montant), v(d.gapd_duree_mois), v(d.gapd),
...
REPARTITION STRICTE ENTRE motif ET calcul (evite la redondance : le motif donne deja le "quand"/"dans quel cas", calcul ne doit QUE donner le "combien") :
- motif: contexte resume ("Conge delivre par le Preneur pour l'expiration de la Neme periode triennale") — c'est ici et dans break_date que vit le "quand"/"dans quel cas", jamais dans calcul.
- calcul: EXCLUSIVEMENT la formule ou la valeur chiffree du remboursement, condensee, JAMAIS de phrase complete recopiant le motif/la condition. Ne jamais commencer par "En cas de..."/"Si le Preneur...": aller directement au montant/formule. Exemples CORRECTS (courts, chiffres/formule seuls): "2 mois de franchise de loyer", "6 mois de loyer HT/HC", "cumul des avantages consentis (franchise, reduction de loyer, participation travaux)", "115 840 € indexes BT01". Exemples INCORRECTS (a ne jamais produire, trop verbeux/redondants avec motif): "En cas de conge delivre par le Preneur pour l'expiration de la premiere periode triennale, il devra rembourser au Bailleur l'equivalent de 2 (deux) mois de franchise" (repete le motif) → devrait etre juste "2 mois de franchise de loyer".
NE PAS INVENTER de montants. Format: [{"break_date":"31/08/2028 ou null","motif":"texte court du contexte","montant":"chiffres bruts ou null","calcul":"formule/montant condense uniquement, sans repeter le contexte","page":7}].`
...
function ConditionsSuspensivesBoxes({ conditions, item }) {
  ...
  const statutStyle = (statut) => {
    ...
    if (s.includes('lev') || s.includes('réalis')) return { cls: 'pill-green', label: statut }
    if (s.includes('cours')) return { cls: 'pill-blue', label: statut }
    return null
  }
  return (
    <div className="gx">
...
// Coupe le libellé du type de bail avant toute citation légale d'articles
// (ex: "Bail commercial soumis aux articles L.145-1 à L.145-60...") — utile
// aussi pour les extractions déjà en base, faites avant l'ajout de la règle
// de concision côté prompt.
function simplifyTypeBail(s) {
  if (!s) return s
  const cut = s.split(/\s+(?:soumis|régi[e]?|conform[ée]ment|relevant)\b.{0,20}\b(?:articles?|L\.|D\.|R\.)/i)[0]
  return (cut || s).trim()
}
...
const PUBLIC_SECTOR_KEYWORDS = [
  ...
  'COMMUNAUTE D\'AGGLOMERATION', 'COMMUNAUTE DE COMMUNES', 'SYNDICAT MIXTE',
...
function ResultsView({ item, parentBailData, onSaveManualDateEffet, onSaveManualDateEffetAvenant, onSaveManualSurface, onSaveManualParking, onSaveManualDureeFerme }) {
  ...
  return (
    <div className="result-body">
              ...
              {primaryDates.map(f => {
                ...
                <div key={f.key} className={`date-card${f.type === 'break' ? ' date-card-break' : ''}`}
                  ...
                  {isEffetCond && f.condition && (
                    <div style={{ fontSize: '11px', color: '#B8860B', marginTop: '4px' }}>
                      À la levée de : {f.condition.libelle || 'condition suspensive'}{f.condition.date_limite ? ` — au plus tard le ${f.condition.date_limite}` : ''}
                    </div>
                  )}
      ...
      {(show('surface_totale_m2') || show('parking_nb_places') || show('rie') || show('classification_batiment')) && (
        <div className="sec">
          <div className="sec-hd"><div className="sec-label">Surfaces</div></div>
          <div className="gx">
            {show('classification_batiment') && d.classification_batiment && <Field label="Classification réglementaire" value={d.classification_batiment} />}
      ...
      {show('depot_garantie_montant') && (
        <div className="sec">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: (d.gapd_montant || d.gapd) ? '1fr 1fr' : '1fr', gap: '10px' }}>
            {(() => {
              // Garde-fou : certaines extractions rattachent par erreur le
              // nombre de mois au dépôt de garantie alors qu'il décrit en
              // réalité la GAPD qui le remplace ("le Preneur substitue au
              // dépôt de garantie une GAPD correspondant à X mois..."). Tant
              // que le bail n'est pas réextrait avec la règle corrigée, on
              // masque durée/montant calculé côté Dépôt de garantie dans ce
              // cas précis — les modalités restent affichées intégralement.
              const dgSubstituee = (d.gapd_montant || d.gapd) && /substitu|remplac/i.test(d.depot_garantie || '')
              return (
                <GuaranteeBox
                  title="Dépôt de garantie"
                  montant={fmtEur(d.depot_garantie_montant) || d.depot_garantie_montant}
                  montantPageField="depot_garantie_montant"
                  duree={dgSubstituee ? null : d.depot_garantie_duree_mois}
                  montantCalcule={dgSubstituee ? null : computeGuaranteeAmount(d.depot_garantie_duree_mois, d.loyer_cours || d.loyer_signature_montant)}
      ...
      {(d.remise_en_etat || d.restitution_etat || d.restitution_delai_edl || d.restitution_conditions_specifiques) && (
        <div className="sec">
          <div className="sec-hd"><div className="sec-label">Conditions de restitution</div></div>
          <div className="gx">
            {d.restitution_etat && <Field label="État de restitution requis" value={d.restitution_etat} verbose />}
            {d.restitution_delai_edl && <Field label="Délai pour l'état des lieux de sortie" value={d.restitution_delai_edl} verbose />}
            {d.restitution_conditions_specifiques && <Field label="Conditions spécifiques" value={d.restitution_conditions_specifiques} verbose />}
            {d.remise_en_etat && <Field label="Remise en état (indemnité forfaitaire à la signature)" value={d.remise_en_etat} verbose />}
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const lastCheckedEmailRef = useRef(null)
  useEffect(() => {
  ...
  useEffect(() => {
    if (!session) { setProfile(null); setAuthLoading(false); lastCheckedEmailRef.current = null; return }
    // Supabase rafraîchit automatiquement le jeton de session dès que l'onglet
    // reprend le focus, même après une coupure d'une seconde — ce qui déclenche
    // cet effet à chaque fois. Sans ce garde-fou, on repasserait l'appli entière
    // par l'écran "Chargement…" (démontage/remontage du contenu, perte du
    // défilement) à chaque changement d'onglet, alors que l'utilisateur n'a pas
    // changé. On ne revérifie donc le profil que si l'email a réellement changé.
    if (session.user.email === lastCheckedEmailRef.current) return
    lastCheckedEmailRef.current = session.user.email
    setAuthLoading(true)
